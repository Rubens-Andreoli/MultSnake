...
public class CrazyBot extends Bot{
    ...
    private int craziness = 2;
    
    public CrazyBot(Snake snake) {
	super(snake);
	...
	switch(snake.getDirection()){
	    case DOWN:	
	    case UP:
		    if(cherry.getLocation().x+random.nextInt(craziness) < snake.getHead().x && !snake.isCollisionAhead(LEFT, 1, avoidPoints)){
			snake.changeDirection(Snake.LEFT);
		    }else if(cherry.getLocation().x+random.nextInt(craziness) > snake.getHead().x && !snake.isCollisionAhead(RIGHT, 1, avoidPoints)){
	    ...
	    case LEFT:
	    case RIGHT:
		    if(cherry.getLocation().y+random.nextInt(craziness) < snake.getHead().y && !snake.isCollisionAhead(UP, 1, avoidPoints)){
			snake.changeDirection(Snake.UP);
		    }else if(cherry.getLocation().y+random.nextInt(craziness) > snake.getHead().y && !snake.isCollisionAhead(DOWN, 1, avoidPoints)){
